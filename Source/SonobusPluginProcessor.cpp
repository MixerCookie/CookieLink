...
namespace {
bool relayPeerDebugEnabled()
{
    auto value = juce::SystemStats::getEnvironmentVariable("COOKIELINK_RELAY_DEBUG", "");
    return value.isNotEmpty() && value != "0";
}

void relayTraceLog(const juce::String& message)
{
    juce::File traceFile("/tmp/cookielink-relay-trace.log");
...
constexpr double kTcpRelayBaselineMs = 50.0;
constexpr double kTcpRelayQueueCongestMs = 60.0;
constexpr double kTcpRelayWriteCongestMs = 25.0;
constexpr double kTcpRelayCongestionHoldMs = 500.0;
constexpr double kTcpRelayBufferAdjustCooldownMs = 1200.0;
constexpr double kTcpRelayStableRestoreMs = 3000.0;
}
...
static String transportModeKey("transportMode");
static String relayUseTlsKey("relayUseTls");
static String relayTlsVerifyKey("relayTlsVerify");
static String relayTlsCaPathKey("relayTlsCaPath");
...
struct SonobusAudioProcessor::EndpointState {
    ...
    String ipaddr;
    int port = 0;
    SonobusAudioProcessor::TransportMode transport = SonobusAudioProcessor::TransportModeUdpP2P;
    uint32_t relayId = 0;
    String relayName;
    TcpRelayClient *relayClient = nullptr;
    
    
    struct sockaddr * getRawAddr() {
        if (transport == SonobusAudioProcessor::TransportModeTcpRelay) {
            return nullptr;
        }
...
struct SonobusAudioProcessor::RemotePeer {
    ...
    int reqRemoteSendFormatIndex = -1;
    int packetsize = 600;
    ...
    bool relayResendConstrained = false;
    int relayNormalResendLimit = -1;
...
static int32_t endpoint_send(void *e, const char *data, int32_t size)
{
    ...
    int result = -1;
    if (endpoint->transport == SonobusAudioProcessor::TransportModeTcpRelay && endpoint->relayClient) {
        result = endpoint->relayClient->sendPacket(endpoint->relayId, data, size) ? size : -1;
    }
    else {
        if (endpoint->peer) {
...
{
public:
    SendThread(SonobusAudioProcessor & processor) : Thread("CookieLinkSendThread") , _processor(processor) 
    {}
...
{
public:
    RecvThread(SonobusAudioProcessor & processor) : Thread("CookieLinkRecvThread") , _processor(processor) 
    {}
...
{
public:
    EventThread(SonobusAudioProcessor & processor) : Thread("CookieLinkEventThread") , _processor(processor) 
    {}
...
{
public:
    ServerThread(SonobusAudioProcessor & processor) : Thread("CookieLinkServerThread") , _processor(processor) 
    {}
...
{
public:
    ClientThread(SonobusAudioProcessor & processor) : Thread("CookieLinkClientThread") , _processor(processor) 
    {}
...
mGlobalState("CookieLinkGlobalState"),
mState (*this, &mUndoManager, "CookieLinkAoO",
{
    ...
    std::make_unique<AudioParameterFloat>(ParameterID(paramDefaultPeerLevel, 1),     TRANS ("Default User Level"),    NormalisableRange<float>(0.0,    1.0, 0.0, 0.5), mDefUserLevel.get(), "", AudioProcessorParameter::genericParameter,
                                          [](float v, int maxlen) -> String { return Decibels::toString(Decibels::gainToDecibels(v), 1); },
                                          [](const String& s) -> float { return Decibels::decibelsToGain(s.getFloatValue()); }),
    std::make_unique<AudioParameterBool>(ParameterID(paramSyncMetToHost, 1), TRANS ("Sync to Host"),
#if JucePlugin_Build_AAX
        false
#else
        JUCEApplicationBase::isStandaloneApp() ? false : true
#endif
    ),
...
})
{
    ...
    options.applicationName     = "CookieLink";
    options.filenameSuffix      = ".xml";
    options.osxLibrarySubFolder = "Application Support/CookieLink";
   #if JUCE_LINUX
    options.folderName          = "~/.config/cookielink";
   #else
    options.folderName          = "";
   #endif
    ...
    parentDir = parentDir.getChildFile("CookieLink");
    mDefaultRecordDir = URL(parentDir);
    mLastBrowseDir = mDefaultRecordDir.getLocalFile().getFullPathName();
#endif
...
void SonobusAudioProcessor::moveOldMisplacedFiles()
{
    ...
    dummyoptions.osxLibrarySubFolder = "Application Support/CookieLink";
   #if JUCE_LINUX
    dummyoptions.folderName          = "~/.config/cookielink";
   #else
...
void SonobusAudioProcessor::setTransportMode(TransportMode mode)
{
    if ((TransportMode)mTransportMode.get() == mode) {
        return;
    }

    if (isConnectedToServer()) {
        // disconnect using current transport before switching
        disconnectFromServer("Processor::setTransportMode");
    }
...
void SonobusAudioProcessor::initializeAoo(int udpPort)
{
    ...
    mServerEndpoint->processor = this;
    mServerEndpoint->owner = mUdpSocket.get();
    mServerEndpoint->transport = TransportModeUdpP2P;
...
{
    disconnectFromServer("Processor::cleanupAoo");
...
{
    if (isTcpRelayMode()) {
        relayTraceLog("connectToServer relay host=" + host
                      + " port=" + juce::String(port)
                      + " user=" + username);
        if (!mRelayClient) {
            mRelayClient = std::make_unique<TcpRelayClient>(*this);
        }
        mRelayClient->setTlsOptions(mRelayUseTls, mRelayTlsVerify, mRelayTlsCaPath);
        mRelayClient->setWatchPublicGroups(mWatchPublicGroups);
        if (!mRecoveringFromServerLoss) {
            removeAllRemotePeers();
        }
        mCurrentUsername = username;
        return mRelayClient->connectToServer(host, port, username, passwd);
    }
...
bool SonobusAudioProcessor::disconnectFromServer(const String& reason)
{
    if (isTcpRelayMode()) {
        relayTraceLog("disconnectFromServer relay reason=" + reason);
        bool wasConnected = false;
        String groupName;
        {
            const ScopedLock sl (mClientLock);
            wasConnected = mIsConnectedToRelay;
            groupName = mCurrentJoinedGroup;
        }
        juce::String logLine = "disconnectFromServer(tcp relay) called: connected="
                               + juce::String(wasConnected ? "yes" : "no")
                               + " group=" + groupName;
        ...
        {
            const ScopedLock sl (mClientLock);
            mIsConnectedToRelay = false;
            mSessionConnectionStamp = 0.0;
            mCurrentJoinedGroup.clear();
            mPendingRelayGroup.clear();
            mPendingRelayGroupIsPublic = false;
        }
        mPendingReconnect = false;
        mRecoveringFromServerLoss = false;
        if (mReconnectTimer.isTimerRunning()) {
...
bool SonobusAudioProcessor::setWatchPublicGroups(bool flag)
{
    if (isTcpRelayMode()) {
        ...
        if (mRelayClient) {
            mRelayClient->setWatchPublicGroups(flag);
        }
        if (!flag) {
            const ScopedLock sl (mPublicGroupsLock);
            mPublicGroupInfos.clear();
...
{
    if (isTcpRelayMode()) {
        if (!mRelayClient) return false;
        {
            const ScopedLock sl (mClientLock);
            mPendingRelayGroup = group;
            mPendingRelayGroupIsPublic = isPublic;
        }
        return mRelayClient->joinGroup(group, groupsecret, isPublic);
    }
    if (!mAooClient) return false;
...
bool SonobusAudioProcessor::leaveServerGroup(const String & group)
{
    if (isTcpRelayMode()) {
        ...
        clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientGroupLeft, this, true, group, "");
        return true;
    }
    if (!mAooClient) return false;
...
bool SonobusAudioProcessor::isRelayGroupJoinPending() const
{
    const ScopedLock sl (mClientLock);
    return mPendingRelayGroup.isNotEmpty();
}
...
{
    if (formatIndex < 0 || formatIndex >= mAudioFormats.size() || index >= mRemotePeers.size()) return false;
    ...
    if (remote->remoteSourceId == AOO_ID_NONE || remote->recvChannels <= 0)
        return true;
...
SonobusAudioProcessor::EndpointState * SonobusAudioProcessor::findOrAddEndpoint(const String & host, int port)
{
    ...
    if (!endpoint) {
        ...
        endpoint->transport = TransportModeUdpP2P;
        endpoint->relayClient = nullptr;
        endpoint->relayId = 0;
        endpoint->relayName.clear();
        DBG("Added new endpoint for " << host << ":" << port);
    }
    return endpoint;
}

SonobusAudioProcessor::EndpointState * SonobusAudioProcessor::findOrAddRelayEndpoint(uint32_t relayId, const String & displayName)
{
    const ScopedLock sl (mEndpointsLock);

    EndpointState * endpoint = nullptr;
    for (auto ep : mEndpoints) {
        if (ep->transport == TransportModeTcpRelay && ep->relayId == relayId) {
            endpoint = ep;
            break;
        }
    }
    ...
    if (!endpoint) {
        String label = displayName.isNotEmpty() ? displayName : ("relay-" + String(relayId));
        endpoint = mEndpoints.add(new EndpointState(label, 0));
        endpoint->processor = this;
        endpoint->owner = mUdpSocket.get();
        endpoint->transport = TransportModeTcpRelay;
        endpoint->relayClient = mRelayClient.get();
        endpoint->relayId = relayId;
        endpoint->relayName = displayName;
        endpoint->peer.reset();
        DBG("Added relay endpoint id: " << String(relayId) << " name: " << displayName);
    }
    else if (displayName.isNotEmpty()) {
        endpoint->relayName = displayName;
        endpoint->ipaddr = displayName;
    }
...
void SonobusAudioProcessor::doReceiveData()
{
    ...
    handleIncomingPacket(endpoint, buf, nbytes);

}

void SonobusAudioProcessor::handleIncomingPacket(EndpointState * endpoint, const char * buf, int nbytes)
{
    ...
    {
        {
            ...
            if (type == AOO_TYPE_SINK){
                ...
                if (mAooClient && endpoint->transport == TransportModeUdpP2P) {
                    auto * rawaddr = endpoint->getRawAddr();
                    if (rawaddr) {
                        mAooClient->handle_message(buf, nbytes, rawaddr);
                    }
                }
                ...
                DBG("CookieLink bug: unknown aoo type: " << type);
            }
        }
        ...
        DBG("CookieLink: not a valid AOO message!");
    }
}

void SonobusAudioProcessor::handleRelayData(uint32_t srcId, const char * data, int size)
{
    if (size <= 0 || data == nullptr) {
        return;
    }

    EndpointState * endpoint = findOrAddRelayEndpoint(srcId);
    if (!endpoint) {
        return;
    }

    handleIncomingPacket(endpoint, data, size);
}

// XXX
// all of this should be refactored into its own class

#define COOKIELINK_MSG_DOMAIN "/sb"
#define COOKIELINK_MSG_DOMAIN_LEN 3

#define COOKIELINK_MSG_PEERINFO "/pinfo"
#define COOKIELINK_MSG_PEERINFO_LEN 6
#define COOKIELINK_FULLMSG_PEERINFO COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_PEERINFO

#define COOKIELINK_MSG_LAYOUTINFO "/clayinfo"
#define COOKIELINK_MSG_LAYOUTINFO_LEN 9
#define COOKIELINK_FULLMSG_LAYOUTINFO COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_LAYOUTINFO

#define COOKIELINK_MSG_CHAT "/chat"
#define COOKIELINK_MSG_CHAT_LEN 5
#define COOKIELINK_FULLMSG_CHAT COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_CHAT

#define COOKIELINK_MSG_PING "/ping"
#define COOKIELINK_MSG_PING_LEN 5
#define COOKIELINK_FULLMSG_PING COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_PING

#define COOKIELINK_MSG_PINGACK "/pngack"
#define COOKIELINK_MSG_PINGACK_LEN 7
#define COOKIELINK_FULLMSG_PINGACK COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_PINGACK

#define COOKIELINK_MSG_REQLATINFO "/reqlatinfo"
#define COOKIELINK_MSG_REQLATINFO_LEN 12
#define COOKIELINK_FULLMSG_REQLATINFO COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_REQLATINFO

#define COOKIELINK_MSG_LATINFO "/latinfo"
#define COOKIELINK_MSG_LATINFO_LEN 8
#define COOKIELINK_FULLMSG_LATINFO COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_LATINFO

#define COOKIELINK_MSG_SUGGESTLAT "/suggestlat"
#define COOKIELINK_MSG_SUGGESTLAT_LEN 11
#define COOKIELINK_FULLMSG_SUGGESTLAT COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_SUGGESTLAT

#define COOKIELINK_MSG_BLOCKEDINFO "/blockedinfo"
#define COOKIELINK_MSG_BLOCKEDINFO_LEN 13
#define COOKIELINK_FULLMSG_BLOCKEDINFO COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_BLOCKEDINFO

#define COOKIELINK_MSG_SUGGEST_GROUP "/suggestgroup"
#define COOKIELINK_MSG_SUGGEST_GROUP_LEN 14
#define COOKIELINK_FULLMSG_SUGGEST_GROUP COOKIELINK_MSG_DOMAIN COOKIELINK_MSG_SUGGEST_GROUP


enum {
    COOKIELINK_MSGTYPE_UNKNOWN = 0,
    COOKIELINK_MSGTYPE_PEERINFO,
    COOKIELINK_MSGTYPE_LAYOUTINFO,
    COOKIELINK_MSGTYPE_CHAT,
    COOKIELINK_MSGTYPE_PING,
    COOKIELINK_MSGTYPE_PINGACK,
    COOKIELINK_MSGTYPE_REQLATINFO,
    COOKIELINK_MSGTYPE_LATINFO,
    COOKIELINK_MSGTYPE_SUGGESTLAT,
    COOKIELINK_MSGTYPE_BLOCKEDINFO,
    COOKIELINK_MSGTYPE_SUGGESTGROUP
};

static int32_t cookieLinkOscParsePattern(const char *msg, int32_t n, int32_t & rettype)
{
    int32_t offset = 0;
    if (n >= COOKIELINK_MSG_DOMAIN_LEN
        && !memcmp(msg, COOKIELINK_MSG_DOMAIN, COOKIELINK_MSG_DOMAIN_LEN))
    {
        offset += COOKIELINK_MSG_DOMAIN_LEN;

        if (n >= (offset + COOKIELINK_MSG_PING_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_PING, COOKIELINK_MSG_PING_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_PING;
            offset += COOKIELINK_MSG_PING_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_PINGACK_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_PINGACK, COOKIELINK_MSG_PINGACK_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_PINGACK;
            offset += COOKIELINK_MSG_PINGACK_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_PEERINFO_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_PEERINFO, COOKIELINK_MSG_PEERINFO_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_PEERINFO;
            offset += COOKIELINK_MSG_PEERINFO_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_LAYOUTINFO_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_LAYOUTINFO, COOKIELINK_MSG_LAYOUTINFO_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_LAYOUTINFO;
            offset += COOKIELINK_MSG_LAYOUTINFO_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_CHAT_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_CHAT, COOKIELINK_MSG_CHAT_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_CHAT;
            offset += COOKIELINK_MSG_CHAT_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_REQLATINFO_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_REQLATINFO, COOKIELINK_MSG_REQLATINFO_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_REQLATINFO;
            offset += COOKIELINK_MSG_REQLATINFO_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_LATINFO_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_LATINFO, COOKIELINK_MSG_LATINFO_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_LATINFO;
            offset += COOKIELINK_MSG_LATINFO_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_SUGGESTLAT_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_SUGGESTLAT, COOKIELINK_MSG_SUGGESTLAT_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_SUGGESTLAT;
            offset += COOKIELINK_MSG_SUGGESTLAT_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_BLOCKEDINFO_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_BLOCKEDINFO, COOKIELINK_MSG_BLOCKEDINFO_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_BLOCKEDINFO;
            offset += COOKIELINK_MSG_BLOCKEDINFO_LEN;
            return offset;
        }
        else if (n >= (offset + COOKIELINK_MSG_SUGGEST_GROUP_LEN)
            && !memcmp(msg + offset, COOKIELINK_MSG_SUGGEST_GROUP, COOKIELINK_MSG_SUGGEST_GROUP_LEN))
        {
            rettype = COOKIELINK_MSGTYPE_SUGGESTGROUP;
            offset += COOKIELINK_MSG_SUGGEST_GROUP_LEN;
            return offset;
        }
        else {
            return 0;
...
bool SonobusAudioProcessor::handleOtherMessage(EndpointState * endpoint, const char *msg, int32_t n)
{
    ...
    int32_t type = COOKIELINK_MSGTYPE_UNKNOWN;
    int32_t onset = 0;

    if (! (onset = cookieLinkOscParsePattern(msg, n, type))) {
        return false;
    }
    ...
    try {
        ...
        if (type == COOKIELINK_MSGTYPE_PING) {
            ...
            try {
                outmsg << osc::BeginMessage(COOKIELINK_FULLMSG_PINGACK)
        ...
        else if (type == COOKIELINK_MSGTYPE_PINGACK) {
            // received from the other side
        ...
        else if (type == COOKIELINK_MSGTYPE_PEERINFO) {
        ...
        else if (type == COOKIELINK_MSGTYPE_LAYOUTINFO) {
        ...
        else if (type == COOKIELINK_MSGTYPE_CHAT) {
            // layout info message arguments:
        ...
        else if (type == COOKIELINK_MSGTYPE_REQLATINFO) {
            // received from the other side
            // args: none
            ...
            if (!isAddressBlocked(endpoint->ipaddr)) {
                ...
                try {
                    outmsg << osc::BeginMessage(COOKIELINK_FULLMSG_LATINFO)
        ...
        else if (type == COOKIELINK_MSGTYPE_LATINFO) {
            // peerinfo message arguments:
            // blob containing JSON
            auto it = message.ArgumentsBegin();
        ...
        else if (type == COOKIELINK_MSGTYPE_SUGGESTLAT) {
            // received from the other side
        ...
        else if (type == COOKIELINK_MSGTYPE_SUGGESTGROUP) {
            // received from the other side
        ...
        else if (type == COOKIELINK_MSGTYPE_BLOCKEDINFO) {
...
bool SonobusAudioProcessor::sendChatEvent(const SBChatEvent & event)
{
    ...
    try {
        ...
        msg << osc::BeginMessage(COOKIELINK_FULLMSG_CHAT)
...
void SonobusAudioProcessor::sendBlockedInfoMessage(EndpointState *endpoint, bool blocked)
{
    ...
    try {
        ...
        outmsg << osc::BeginMessage(COOKIELINK_FULLMSG_BLOCKEDINFO)
...
void SonobusAudioProcessor::sendReqLatInfoToAll()
{
    ...
    try {
        ...
        msg << osc::BeginMessage(COOKIELINK_FULLMSG_REQLATINFO)
        << osc::EndMessage;
...
void SonobusAudioProcessor::sendLatencyMatchToAll(float latency)
{
    ...
    try {
        ...
        msg << osc::BeginMessage(COOKIELINK_FULLMSG_SUGGESTLAT)
        << mCurrentUsername.toRawUTF8()
        << latency
        << osc::EndMessage;
...
void SonobusAudioProcessor::suggestNewGroupToPeers(const String & group, const String & groupPass, const StringArray & peernames, bool ispublic)
{
    ...
    try {
        msg << osc::BeginMessage(COOKIELINK_FULLMSG_SUGGEST_GROUP)
        << osc::Blob(jsonstr.toRawUTF8(), (int) jsonstr.getNumBytesAsUTF8())
        << osc::EndMessage;
    }
    catch (const osc::Exception& e){
...
void SonobusAudioProcessor::sendRemotePeerInfoUpdate(int index, RemotePeer * topeer)
{
    ...
    for (int i=0;  i < mRemotePeers.size(); ++i) {
        ...
        try {
            msg << osc::BeginMessage(COOKIELINK_FULLMSG_PEERINFO)
            << osc::Blob(jsonstr.toRawUTF8(), (int) jsonstr.getNumBytesAsUTF8())
            << osc::EndMessage;
        }
        catch (const osc::Exception& e){
...
void SonobusAudioProcessor::doSendData()
{
    ...
    if (mPendingUnmute.get() && mPendingUnmuteAtStamp < Time::getMillisecondCounter() ) {
        DBG("UNMUTING ALL");
        mMainRecvMute = false;
        mPendingHostMainRecvMute = false;
        mNotifyHostMainRecvMutePending = true;
        triggerAsyncUpdate();
...
void SonobusAudioProcessor::handleEvents()
{
    ...
    updateRelayCongestionAdaptation();
}

SonobusAudioProcessor::RelayRuntimeStats SonobusAudioProcessor::getRelayRuntimeStats()
{
    RelayRuntimeStats ret;
    if (mRelayClient) {
        const auto stats = mRelayClient->getStats();
        ret.active = stats.connected;
        ret.congested = stats.congested;
        ret.queuedFrames = stats.queuedFrames;
        ret.queuedBytes = stats.queuedBytes;
        ret.droppedQueuedPackets = stats.droppedQueuedPackets;
        ret.droppedStalePackets = stats.droppedStalePackets;
        ret.oldestQueuedMs = stats.oldestQueuedMs;
        ret.lastWriteBlockMs = stats.lastWriteBlockMs;
        ret.avgWriteBlockMs = stats.avgWriteBlockMs;
        ret.maxWriteBlockMs = stats.maxWriteBlockMs;
    }
    return ret;
}

void SonobusAudioProcessor::setRelayPeerResendConstrained(RemotePeer * peer, bool constrained)
{
    if (peer == nullptr || peer->oursink == nullptr) {
        return;
    }
    if (peer->relayResendConstrained == constrained) {
        return;
    }

    if (peer->relayNormalResendLimit < 0) {
        int32_t currentLimit = AOO_RESEND_LIMIT;
        if (peer->oursink->get_resend_limit(currentLimit) >= 0) {
            peer->relayNormalResendLimit = currentLimit;
        } else {
            peer->relayNormalResendLimit = AOO_RESEND_LIMIT;
        }
    }

    const int32_t limit = constrained ? 0 : static_cast<int32_t>(juce::jmax(0, peer->relayNormalResendLimit));
    peer->oursink->set_resend_limit(limit);
    if (peer->latencysink) {
        peer->latencysink->set_resend_limit(limit);
    }
    if (peer->echosink) {
        peer->echosink->set_resend_limit(limit);
    }
    peer->relayResendConstrained = constrained;
}

void SonobusAudioProcessor::updateRelayCongestionAdaptation()
{
    if (!mRelayClient || !isTcpRelayMode()) {
        mRelayCongestionStartedMs = 0.0;
        mRelayLastCongestedMs = 0.0;
        mRelayLastDroppedPackets = 0;
        mRelayWasCongested = false;
        return;
    }

    const auto stats = mRelayClient->getStats();
    const int totalDropped = stats.droppedQueuedPackets + stats.droppedStalePackets;
    const bool droppedSinceLastPoll = totalDropped > mRelayLastDroppedPackets;
    mRelayLastDroppedPackets = totalDropped;

    const bool congested = stats.congested
                           || droppedSinceLastPoll
                           || stats.oldestQueuedMs >= kTcpRelayQueueCongestMs
                           || stats.lastWriteBlockMs >= kTcpRelayWriteCongestMs
                           || stats.avgWriteBlockMs >= (kTcpRelayWriteCongestMs * 0.6);
    const double nowMs = Time::getMillisecondCounterHiRes();
    if (congested) {
        if (mRelayCongestionStartedMs <= 0.0) {
            mRelayCongestionStartedMs = nowMs;
        }
        mRelayLastCongestedMs = nowMs;
    } else {
        mRelayCongestionStartedMs = 0.0;
    }

    bool changedPeerInfo = false;
    const bool sustainedCongestion = congested && nowMs - mRelayCongestionStartedMs >= kTcpRelayCongestionHoldMs;
    const bool canAdjustBuffer = sustainedCongestion
                                 && nowMs - mRelayLastBufferAdjustMs >= kTcpRelayBufferAdjustCooldownMs;
    const bool stableAfterCongestion = !congested
                                       && mRelayWasCongested
                                       && mRelayLastCongestedMs > 0.0
                                       && nowMs - mRelayLastCongestedMs >= kTcpRelayStableRestoreMs;

    for (auto* peer : mRemotePeers) {
        if (peer == nullptr || peer->endpoint == nullptr || peer->endpoint->transport != TransportModeTcpRelay) {
            continue;
        }

        if (peer->autosizeBufferMode != AutoNetBufferModeOff) {
            const float relayBaseline = static_cast<float>(kTcpRelayBaselineMs);
            if (peer->buffertimeMs < relayBaseline || peer->netBufAutoBaseline < relayBaseline) {
                peer->buffertimeMs = jmax(peer->buffertimeMs, relayBaseline);
                peer->netBufAutoBaseline = jmax(peer->netBufAutoBaseline, relayBaseline);
                ...
                changedPeerInfo = true;
            }
        }

        if (congested) {
            setRelayPeerResendConstrained(peer, true);
        } else if (stableAfterCongestion) {
            setRelayPeerResendConstrained(peer, false);
        }

        if (canAdjustBuffer && peer->autosizeBufferMode != AutoNetBufferModeOff && !peer->latencyMatched) {
            const float blockMs = static_cast<float>(1000.0 * currSamplesPerBlock / getSampleRate());
            const float maxBufferMs = static_cast<float>(mMaxBufferTime.get() * 1000.0);
            const float nextBufferMs = jmin(maxBufferMs, jmax(peer->buffertimeMs, static_cast<float>(kTcpRelayBaselineMs)) + blockMs);
            if (nextBufferMs > peer->buffertimeMs + 0.1f) {
                peer->buffertimeMs = nextBufferMs;
                peer->netBufAutoBaseline = jmax(peer->netBufAutoBaseline, static_cast<float>(kTcpRelayBaselineMs));
                peer->totalEstLatency = peer->smoothPingTime.xbar + 2 * peer->buffertimeMs + blockMs;
                peer->oursink->set_buffersize(peer->buffertimeMs);
                peer->echosink->set_buffersize(peer->buffertimeMs);
                peer->latencysink->set_buffersize(peer->buffertimeMs);
                peer->latencyDirty = true;
    ...
    if (canAdjustBuffer) {
        mRelayLastBufferAdjustMs = nowMs;
    }

    if (changedPeerInfo) {
        sendRemotePeerInfoUpdate(-1);
    }

    if (stableAfterCongestion) {
        mRelayWasCongested = false;
        mRelayLastCongestedMs = 0.0;
    } else if (congested) {
        mRelayWasCongested = true;
    }
}

void SonobusAudioProcessor::applyMasterOutputLimiter(AudioBuffer<float>& buffer, int numSamples)
{
    constexpr float limitThreshold = 0.98f;
    constexpr float limitCeiling = 0.999f;
    bool limited = false;
    bool clipped = false;
    ...
    for (int ch = 0; ch < channels; ++ch) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            clipped = clipped || absx > 1.0f;
            if (absx <= limitThreshold) {
                continue;
            }

            limited = true;
            const float sign = x < 0.0f ? -1.0f : 1.0f;
            const float over = (absx - limitThreshold) / (1.0f - limitThreshold);
            const float shaped = limitThreshold + (limitCeiling - limitThreshold) * std::tanh(over);
            samples[i] = sign * juce::jmin(limitCeiling, shaped);
        }
    }

    if (limited) {
        mMasterLimiterHoldBlocks = 24;
    } else if (mMasterLimiterHoldBlocks.get() > 0) {
        mMasterLimiterHoldBlocks = mMasterLimiterHoldBlocks.get() - 1;
    }

    if (clipped) {
        mMasterClipHoldBlocks = 48;
    } else if (mMasterClipHoldBlocks.get() > 0) {
        mMasterClipHoldBlocks = mMasterClipHoldBlocks.get() - 1;
    }
}
...
void SonobusAudioProcessor::sendPingEvent(RemotePeer * peer)
{
    ...
    try {
        outmsg << osc::BeginMessage(COOKIELINK_FULLMSG_PING)
        << osc::TimeTag(tt)
        << osc::EndMessage;
    }
    catch (const osc::Exception& e){
        ...
        case AOO_SOURCE_ADD_EVENT:
        {
            ...
            if (peer) {
                ...
                if (mAooDummySource->get_id(dummyid) && dummyid == e->id ) {
                    ...
                    if (peer->recvAllow && peer->reqRemoteSendFormatIndex >= 0)
                    {
                        int pi = -1;
                        {
                            const ScopedReadLock sl (mCoreLock);
                            pi = mRemotePeers.indexOf (peer);
                        }
                        if (pi >= 0)
                            setRequestRemotePeerSendAudioCodecFormat (pi, peer->reqRemoteSendFormatIndex);
                    }
            ...
            if (peer) {
                ...
                if (peer->oursink->get_source_format(e->endpoint, e->id, f) > 0) {
                    ...
                    if (codec == CodecOpus) {
                        ...
                        actualRecvFormatIndex = findFormatIndex(codec, fmt->bitrate / fmt->header.nchannels, 0);
                    } else {
                        ...
                        actualRecvFormatIndex = findFormatIndex(codec, 0, bitdepth);
                    }

                    if (peer->reqRemoteSendFormatIndex >= 0 && peer->reqRemoteSendFormatIndex != actualRecvFormatIndex)
                    {
                        int pi = -1;
                        {
                            const ScopedReadLock sl (mCoreLock);
                    ...
                    else if (peer->reqRemoteSendFormatIndex == actualRecvFormatIndex)
                    {
                        peer->reqRemoteSendFormatIndex = -1;
                    }
...
void SonobusAudioProcessor::handleRelayConnected(bool success, const String & errmesg)
{
    relayTraceLog("handleRelayConnected success=" + juce::String(success ? "yes" : "no")
                  + " err=" + errmesg);
    {
        const ScopedLock sl (mClientLock);
        mIsConnectedToRelay = success;
        mSessionConnectionStamp = success ? Time::getMillisecondCounterHiRes() : 0.0;
        if (!success) {
            mCurrentJoinedGroup.clear();
        }
    }
    ...
    if (mPendingReconnect) {
        if (success && mPendingReconnectInfo.groupName.isNotEmpty()) {
            mPendingReconnectInfo.timestamp = Time::getCurrentTime().toMilliseconds();
            addRecentServerConnectionInfo(mPendingReconnectInfo);
            setWatchPublicGroups(false);
            joinServerGroup(mPendingReconnectInfo.groupName, mPendingReconnectInfo.groupPassword, mPendingReconnectInfo.groupIsPublic);
    ...
    if (success && mReconnectTimer.isTimerRunning()) {
        mReconnectTimer.stopTimer();
        mRecoveringFromServerLoss = false;
    }

    clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientConnected, this, success, errmesg);
}

void SonobusAudioProcessor::handleRelayDisconnected(const String & errmesg)
{
    relayTraceLog("handleRelayDisconnected err=" + errmesg);
    const bool shouldReconnect = mCurrentJoinedGroup.isNotEmpty() && mReconnectAfterServerLoss.get() && !mReconnectTimer.isTimerRunning();
    ...
    clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientDisconnected, this, true, errmesg);
}

void SonobusAudioProcessor::handleRelayGroupJoined(bool success, const String & errmesg)
{
    String requestedGroup;
    String currentGroup;
    {
        const ScopedLock sl (mClientLock);
        requestedGroup = mPendingRelayGroup;
        currentGroup = mCurrentJoinedGroup;

        if (success && requestedGroup.isNotEmpty()) {
            mCurrentJoinedGroup = requestedGroup;
            currentGroup = requestedGroup;
            mSessionConnectionStamp = Time::getMillisecondCounterHiRes();
        }

        if (requestedGroup.isNotEmpty()) {
            mPendingRelayGroup.clear();
            mPendingRelayGroupIsPublic = false;
        }
    }

    const String eventGroup = requestedGroup.isNotEmpty() ? requestedGroup : currentGroup;
    clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientGroupJoined, this, success, eventGroup, errmesg);
}

void SonobusAudioProcessor::handleRelayPeerJoin(uint32_t peerId, const String & user)
{
    int peersBefore = 0;
    if (relayPeerDebugEnabled()) {
        const ScopedReadLock sl (mCoreLock);
        peersBefore = mRemotePeers.size();
        juce::Logger::writeToLog("relay peer-join event id=" + juce::String(peerId)
                                 + " user=" + user
                                 + " group=" + mCurrentJoinedGroup
                                 + " autoconnect=" + juce::String(mAutoconnectGroupPeers ? "yes" : "no")
                                 + " peers=" + juce::String(peersBefore));
    }

    EndpointState * endpoint = findOrAddRelayEndpoint(peerId, user);
    if (!endpoint) {
        if (relayPeerDebugEnabled()) {
            juce::Logger::writeToLog("relay peer-join event id=" + juce::String(peerId)
                                     + " user=" + user + " failed: endpoint missing");
        }
        return;
    }

    if (mAutoconnectGroupPeers) {
        connectRemotePeerRelay(peerId, user, mCurrentJoinedGroup, !mMainRecvMute.get());
    }

    if (relayPeerDebugEnabled()) {
        int peersAfter = 0;
        const ScopedReadLock sl (mCoreLock);
        peersAfter = mRemotePeers.size();
        if (peersAfter != peersBefore) {
            juce::Logger::writeToLog("relay peer-join event id=" + juce::String(peerId)
                                     + " peers now=" + juce::String(peersAfter));
        }
    }

    clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientPeerJoined, this, mCurrentJoinedGroup, user);
}

void SonobusAudioProcessor::handleRelayPeerLeave(uint32_t peerId)
{
    int peersBefore = 0;
    if (relayPeerDebugEnabled()) {
        const ScopedReadLock sl (mCoreLock);
    ...
    String username;
    {
        const ScopedLock sl (mEndpointsLock);
        for (auto ep : mEndpoints) {
            if (ep->transport == TransportModeTcpRelay && ep->relayId == peerId) {
                endpoint = ep;
                username = ep->relayName;
                break;
            }
        }
    }
    ...
    if (relayPeerDebugEnabled()) {
        ...
        const ScopedReadLock sl (mCoreLock);
        peersAfter = mRemotePeers.size();
        juce::Logger::writeToLog("relay peer-leave event id=" + juce::String(peerId)
                                 + " user=" + username
                                 + " peers " + juce::String(peersBefore)
                                 + "->" + juce::String(peersAfter));
    }

    clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientPeerLeft, this, mCurrentJoinedGroup, username);
}

void SonobusAudioProcessor::handleRelayError(const String & errmesg)
{
    clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientError, this, errmesg);
}

void SonobusAudioProcessor::handleRelayPublicGroups(const StringArray & groups, const Array<int> & counts)
{
    StringArray removedGroups;

    {
        const ScopedLock sl (mPublicGroupsLock);
        for (auto & item : mPublicGroupInfos) {
            removedGroups.add(item.first);
        }

        mPublicGroupInfos.clear();

        for (int i = 0; i < groups.size(); ++i) {
            AooPublicGroupInfo info;
            info.groupName = groups[i];
            info.activeCount = (i < counts.size() ? counts[i] : 0);
            info.timestamp = Time::getCurrentTime().toMilliseconds();
            mPublicGroupInfos[info.groupName] = info;
        }
    }

    for (int i = 0; i < groups.size(); ++i) {
        clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientPublicGroupModified, this, groups[i], (i < counts.size() ? counts[i] : 0), "");
    }

    for (auto & group : removedGroups) {
        if (!groups.contains(group)) {
            clientListeners.call(&SonobusAudioProcessor::ClientListener::aooClientPublicGroupDeleted, this, group, "");
        }
    }
}
...
int SonobusAudioProcessor::connectRemotePeerRelay(uint32_t relayId, const String & username, const String & groupname, bool reciprocate)
{
    EndpointState * endpoint = findOrAddRelayEndpoint(relayId, username);

    if (!endpoint) {
        DBG("Error getting relay endpoint for id: " << String(relayId));
        return 0;
    }

    RemotePeer * remote = doAddRemotePeerIfNecessary(endpoint, AOO_ID_NONE, username, groupname);
    ...
    if (ret) {
        DBG("Successfully invited relay peer " << String(relayId) << " - ourId " << remote->ourId);
        remote->connected = true;
        remote->invitedPeer = reciprocate;
        if (!mMainSendMute.get()) {
            remote->sendActive = true;
        ...
        DBG("Error inviting relay peer " << String(relayId) << " - ourId " << remote->ourId);
    }
...
String SonobusAudioProcessor::getRemotePeerDisplayName(int index) const
{
    const ScopedReadLock sl (mCoreLock);
    if (index < mRemotePeers.size()) {
        auto * remote = mRemotePeers.getUnchecked(index);
        if (remote->endpoint && remote->endpoint->transport == TransportModeTcpRelay && remote->endpoint->relayId > 0) {
            if (remote->userName.isNotEmpty()) {
                return remote->userName + " @" + String(remote->endpoint->relayId);
            }
            return "relay-" + String(remote->endpoint->relayId);
        }
        return remote->userName;
    }
    return "";
...
void SonobusAudioProcessor::setRemotePeerRecvAllow(int index, bool allow, bool cached)
{
    ...
    if (index < mRemotePeers.size() && index >= 0) {
        ...
        if (cached) {
            ...
            // Must invite/uninvite for both mute and unmute (previously unmute never re-invited).
...
SonobusAudioProcessor::RemotePeer * SonobusAudioProcessor::doAddRemotePeerIfNecessary(EndpointState * endpoint, int32_t ourId, const String & username, const String & groupname)
{
    ...
    if (doadd) {
        ...
        if (endpoint->transport == TransportModeTcpRelay && retpeer->autosizeBufferMode != AutoNetBufferModeOff) {
            retpeer->buffertimeMs = jmax(retpeer->buffertimeMs, static_cast<float>(kTcpRelayBaselineMs));
            retpeer->netBufAutoBaseline = jmax(retpeer->netBufAutoBaseline, static_cast<float>(kTcpRelayBaselineMs));
        }
        ...
        if (username.isNotEmpty() && retpeer->userName.isEmpty()) {
            ...
            if (findAndLoadCacheForPeer(retpeer)) {
                if (retpeer->endpoint->transport == TransportModeTcpRelay && retpeer->autosizeBufferMode != AutoNetBufferModeOff) {
                    retpeer->buffertimeMs = jmax(retpeer->buffertimeMs, static_cast<float>(kTcpRelayBaselineMs));
                    retpeer->netBufAutoBaseline = jmax(retpeer->netBufAutoBaseline, static_cast<float>(kTcpRelayBaselineMs));
                }
...
void SonobusAudioProcessor::updateRemotePeerUserFormat(int index, RemotePeer * onlypeer)
{
    ...
    for (int i=0;  i < mRemotePeers.size(); ++i) {
        ...
        try {
            msg << osc::BeginMessage(COOKIELINK_FULLMSG_LAYOUTINFO)
            << peer->remoteSinkId
...
bool SonobusAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    ...
    auto isMainSetSupported = [] (const AudioChannelSet& set)
    {
        return set == AudioChannelSet::mono()
            || set == AudioChannelSet::stereo()
            || set == AudioChannelSet::disabled();
    };

    if (! isMainSetSupported (layouts.getMainInputChannelSet())
        || ! isMainSetSupported (layouts.getMainOutputChannelSet()))
        return false;
...
void SonobusAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
...
#if JucePlugin_Build_AAX
        // Pro Tools/AAX is especially sensitive to allocation or graph reconfiguration on
        // the realtime callback. Defer the expensive resize/setup work to the send thread
        // and keep this block silent rather than risking an AAE-9173 CPU spike.
        mNeedsSampleSetup = true;
        buffer.clear();
        return;
#else
    ...
    if (syncmethost) {
        if (rposInfo && fabs(useBpm - mMetTempo.get()) > 0.001) {
            mMetTempo = useBpm;
            mPendingHostMetTempo = useBpm;
            mNotifyHostMetTempoPending = true;
            triggerAsyncUpdate();
        }
    }
#endif
...
#if ! JucePlugin_Build_AAX
        mPendingHostMetEnabled = metenabled;
        mNotifyHostMetEnabledPending = true;
        triggerAsyncUpdate();
#endif
    }
    ...
    // send meter post panning, post file/met, and post send level
    if (fabsf(wetnow - mLastWet) > 0.00001f) {
        for (int channel = 0; channel < sendWorkBuffer.getNumChannels(); ++channel) {
            sendWorkBuffer.applyGainRamp(channel, 0, numSamples, mLastWet, wetnow);
        }
    } else {
        for (int channel = 0; channel < sendWorkBuffer.getNumChannels(); ++channel) {
            sendWorkBuffer.applyGain(channel, 0, numSamples, wetnow);
        }
    }
    ...
    applyMasterOutputLimiter(buffer, numSamples);
...
{
#if ! JUCE_IOS && ! JUCE_ANDROID
    return new DesktopShellEditor (*this);
#else
    jassertfalse;
    return nullptr;
#endif
...
ValueTree AooServerConnectionInfo::getValueTree() const
{
    ...
    item.setProperty("transportMode", transportMode, nullptr);
...
{    
    ...
    transportMode = item.getProperty("transportMode", transportMode);
}
...
void SonobusAudioProcessor::getStateInformationWithOptions(MemoryBlock& destData, bool includecache, bool includeInputGroups, bool xmlformat)
{
    ...
    extraTree.setProperty(transportModeKey, var((int)mTransportMode.get()), nullptr);
    extraTree.setProperty(relayUseTlsKey, mRelayUseTls, nullptr);
    extraTree.setProperty(relayTlsVerifyKey, mRelayTlsVerify, nullptr);
    extraTree.setProperty(relayTlsCaPathKey, mRelayTlsCaPath, nullptr);
...
void SonobusAudioProcessor::setStateInformationWithOptions (const void* data, int sizeInBytes, bool includecache, bool includeInputGroups, bool xmlformat)
{
    ...
    if (tree.isValid()) {
        ...
        if (extraTree.isValid()) {
            ...
            setTransportMode((TransportMode)(int)extraTree.getProperty(transportModeKey, (int)mTransportMode.get()));
            setRelayUseTls(extraTree.getProperty(relayUseTlsKey, mRelayUseTls));
            setRelayTlsVerify(extraTree.getProperty(relayTlsVerifyKey, mRelayTlsVerify));
            setRelayTlsCaPath(extraTree.getProperty(relayTlsCaPathKey, mRelayTlsCaPath));
...
void SonobusAudioProcessor::handleAsyncUpdate()
{
    if (mNotifyHostMetTempoPending.get()) {
        const auto tempo = mPendingHostMetTempo.get();
        mNotifyHostMetTempoPending = false;
        if (mTempoParameter != nullptr) {
            mTempoParameter->setValueNotifyingHost(mTempoParameter->convertTo0to1(tempo));
        }
    }

    if (mNotifyHostMetEnabledPending.get()) {
        const auto enabled = mPendingHostMetEnabled.get();
        mNotifyHostMetEnabledPending = false;
        if (auto* param = mState.getParameter(paramMetEnabled)) {
            param->setValueNotifyingHost(enabled ? 1.0f : 0.0f);
        }
    }

    if (mNotifyHostMainRecvMutePending.get()) {
        const auto muted = mPendingHostMainRecvMute.get();
        mNotifyHostMainRecvMutePending = false;
...
bool SonobusAudioProcessor::reconnectToMostRecent()
{
    ...
    if (recents.size() > 0) {
        ...
        if (info.serverHost.isNotEmpty() && info.userName.isNotEmpty()) {
            ...
            setTransportMode((TransportMode)info.transportMode);
