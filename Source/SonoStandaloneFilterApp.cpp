...
#if JUCE_ANDROID && (JUCE_USE_ANDROID_OPENSLES || JUCE_USE_ANDROID_OBOE)
...
public:
    SonobusStandaloneFilterApp()
    {
        ...
        options.applicationName     = "CookieLinkApp";
        options.filenameSuffix      = ".settings";
        options.osxLibrarySubFolder = "Application Support/CookieLinkApp";
       #if JUCE_LINUX
        options.folderName          = "~/.config/cookielink-app";
       #else
        options.folderName          = "";
       #endif
    ...
    void handleCommandLine()
    {
        ...
        const String relaySpec("-r|--tcp-relay");
        const String relaySpecDesc("-r|--tcp-relay");
        ...
        app.addCommand ({ relaySpec, relaySpecDesc,
            TRANS("Use TCP relay transport mode for command-line connection options."),
            TRANS("When this is set, the default port becomes 11000 unless you provide a port with -c/--connectionserver."),
            nullptr
        });
        ...
        const bool forceTcpRelay = arglist.removeOptionIfFound(relaySpec);
        if (forceTcpRelay) {
            cmdlineConnInfo.transportMode = SonobusAudioProcessor::TransportModeTcpRelay;
            cmdlineConnInfo.serverPort = DEFAULT_RELAY_SERVER_PORT;
            copyInfo = true;
        }
        ...
        if (connserv.isNotEmpty()) {
            ...
            if (port > 0) {
                ...
                cmdlineConnInfo.serverPort = (cmdlineConnInfo.transportMode == SonobusAudioProcessor::TransportModeTcpRelay)
        ...
        if (!doHeadless) {
            ...
            if (auto * sonoproc = dynamic_cast<SonobusAudioProcessor*>(mainWindow->pluginHolder->processor.get())) {
                if (sonoproc->hasEditor()) {
                    if (auto* ed = sonoproc->createEditorIfNeeded()) {
                        const auto wireStandaloneEditor = [&] (auto* ptr) {
                            ptr->saveSettingsIfNeeded = [this]()
                            {
                            ...
                            if (doInitialConnect) {
                                const bool alreadyConnectedToRequestedGroup =
                                    sonoproc->isConnectedToServer()
                                    && sonoproc->getCurrentJoinedGroup() == cmdlineConnInfo.groupName
                                    && sonoproc->getCurrentUsername() == cmdlineConnInfo.userName
                                    && sonoproc->getTransportMode() == (SonobusAudioProcessor::TransportMode) cmdlineConnInfo.transportMode;

                                if (alreadyConnectedToRequestedGroup) {
                                    DBG("ALREADY CONNECTED INITIAL, COPYING INFO ONLY");
                                    ptr->connectWithInfo (cmdlineConnInfo, false, true);
                                } else {
                                    DBG("CONNECTING INITIAL");
                                    ptr->connectWithInfo (cmdlineConnInfo, false, false);
                                }
                            } else if (copyInfo) {
                                DBG("COPYING INITIAL");
                                ptr->connectWithInfo (cmdlineConnInfo, false, true);
                            ...
                            if (loadSetupFilename.isNotEmpty()) {
                                ...
                                if (setupfile.exists()) {
                                    Thread::sleep (200);
                                    ptr->loadSettingsFromFile (setupfile);
                                } else {
...
#if JucePlugin_Build_Standalone && ! JUCE_IOS && ! JUCE_ANDROID
                        if (auto* shellEditor = dynamic_cast<DesktopShellEditor*> (ed))
                            wireStandaloneEditor (shellEditor);
#endif
                    }
                }
            }
...
#if JucePlugin_Build_Standalone && ! JUCE_IOS && ! JUCE_ANDROID
    void prepareShellForAppExit()
    {
        if (mainWindow.get() == nullptr)
            return;

        if (auto* editor = mainWindow->getEditor())
            if (auto* shell = dynamic_cast<DesktopShellEditor*> (editor))
                shell->prepareForAppExit();
    }
#endif
        ...
        if (mainWindow.get() != nullptr) {
            ...
            if (auto * sonoproc = dynamic_cast<SonobusAudioProcessor*>(mainWindow->pluginHolder->processor.get())) {
                if (sonoproc->hasEditor()) {
                    if (auto* ed = sonoproc->createEditorIfNeeded()) {
#if JucePlugin_Build_Standalone && ! JUCE_IOS && ! JUCE_ANDROID
                        if (auto* shell = dynamic_cast<DesktopShellEditor*>(ed))
                            shell->handleURL (url.toString (true));
#endif
