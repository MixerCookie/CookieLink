...
#define DEFAULT_RELAY_SERVER_PORT 11000
#define DEFAULT_SERVER_HOST "aoo.vsthome.cn"
...
struct AooServerConnectionInfo
{
    ...
    String serverHost;
    int    serverPort;
    int    transportMode = 0; // 0 = UDP P2P, 1 = TCP relay
...
/**
*/
class SonobusAudioProcessor  : public AudioProcessor,
                               public AudioProcessorValueTreeState::Listener,
                               public ChangeListener,
                               public AsyncUpdater,
                               public TcpRelayClientListener
{
public:
    //==============================================================================
    ...
    enum TransportMode {
        TransportModeUdpP2P = 0,
        TransportModeTcpRelay = 1
    };

    void setTransportMode(TransportMode mode);
    TransportMode getTransportMode() const { return (TransportMode)mTransportMode.get(); }
    bool isTcpRelayMode() const { return getTransportMode() == TransportModeTcpRelay; }
    ...
    bool disconnectFromServer(const juce::String& reason = {});
    ...
    void setRelayUseTls(bool flag) { mRelayUseTls = flag; }
    bool getRelayUseTls() const { return mRelayUseTls; }
    void setRelayTlsVerify(bool flag) { mRelayTlsVerify = flag; }
    bool getRelayTlsVerify() const { return mRelayTlsVerify; }
    void setRelayTlsCaPath(const String & path) { mRelayTlsCaPath = path; }
    String getRelayTlsCaPath() const { return mRelayTlsCaPath; }
    ...
    bool isRelayGroupJoinPending() const;
    ...
    String getRemotePeerDisplayName(int index) const;
    ...
    RelayRuntimeStats getRelayRuntimeStats();
    bool isMasterLimiterActive() const { return mMasterLimiterHoldBlocks.get() > 0; }
    bool isMasterClipRecently() const { return mMasterClipHoldBlocks.get() > 0; }
...
private:
    ...
    void doReceiveData();
    void handleIncomingPacket(EndpointState * endpoint, const char * data, int nbytes);
    void doSendData();
    void handleEvents();
    void updateRelayCongestionAdaptation();
    void setRelayPeerResendConstrained(RemotePeer * peer, bool constrained);
    void applyMasterOutputLimiter(AudioBuffer<float>& buffer, int numSamples);

    EndpointState * findOrAddRelayEndpoint(uint32_t relayId, const String & displayName = {});
    int connectRemotePeerRelay(uint32_t relayId, const String & username = "", const String & groupname = "", bool reciprocate=true);

    void handleRelayData(uint32_t srcId, const char * data, int size) override;
    void handleRelayConnected(bool success, const String & errmesg) override;
    void handleRelayDisconnected(const String & errmesg) override;
    void handleRelayGroupJoined(bool success, const String & errmesg) override;
    void handleRelayPeerJoin(uint32_t peerId, const String & user) override;
    void handleRelayPeerLeave(uint32_t peerId) override;
    void handleRelayError(const String & errmesg) override;
    void handleRelayPublicGroups(const StringArray & groups, const Array<int> & counts) override;
    ...
    Atomic<int>   mSendChannels  { 2 }; // 0 is match inputs, 1 is 1, etc
    ...
    Atomic<int> mMasterLimiterHoldBlocks { 0 };
    Atomic<int> mMasterClipHoldBlocks { 0 };
    ...
    std::unique_ptr<TcpRelayClient> mRelayClient;
    Atomic<int> mTransportMode { TransportModeUdpP2P };
    bool mIsConnectedToRelay = false;
    String mPendingRelayGroup;
    bool mPendingRelayGroupIsPublic = false;
    bool mRelayUseTls = false;
    bool mRelayTlsVerify = false;
    String mRelayTlsCaPath;
    int mRelayLastDroppedPackets = 0;
    double mRelayCongestionStartedMs = 0.0;
    double mRelayLastCongestedMs = 0.0;
    double mRelayLastBufferAdjustMs = 0.0;
    bool mRelayWasCongested = false;
    ...
    Atomic<bool> mNotifyHostMetTempoPending { false };
    Atomic<bool> mNotifyHostMetEnabledPending { false };
    Atomic<bool> mNotifyHostMainRecvMutePending { false };
    Atomic<double> mPendingHostMetTempo { 100.0f };
    Atomic<bool> mPendingHostMetEnabled { false };
    Atomic<bool> mPendingHostMainRecvMute { false };
