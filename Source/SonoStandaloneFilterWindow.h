...
public:
    ...
    */
    ...
    void savePluginState()
    {
        ...
        {
            auto * cookieLinkProcessor = dynamic_cast<SonobusAudioProcessor*>(processor.get());

            bool usexmlstate = cookieLinkProcessor != nullptr;

            MemoryBlock data;

            if (usexmlstate && cookieLinkProcessor) {
                cookieLinkProcessor->getStateInformationWithOptions (data, true, true, usexmlstate);
    ...
    void reloadPluginState()
    {
        if (settings != nullptr)
        {
            ...
            if (cookieLinkProcessor != nullptr && settings->containsKey("filterStateXML")) {
                String filtxml = settings->getValue ("filterStateXML");
                data.replaceWith(filtxml.toUTF8(), filtxml.getNumBytesAsUTF8());
                if (data.getSize() > 0) {
                    cookieLinkProcessor->setStateInformationWithOptions (data.getData(), (int) data.getSize(), true, true, true);
                    return;
                }
            }
...
private:
    ...
    public:
              ...
              editor (owner.getAudioProcessor()->createEditorIfNeeded())
        {
            ...
            if (editor != nullptr)
            {
                if (auto * shellEditor = dynamic_cast<DesktopShellEditor*>(editor.get())) {
                    shellEditor->getAudioDeviceManager = [this]() { return &owner.getDeviceManager();  };
                    shellEditor->isInterAppAudioConnected = [this]() { return owner.pluginHolder->isInterAppAudioConnected();  };
                    shellEditor->getIAAHostIcon = [this](int size) { return owner.pluginHolder->getIAAHostIcon(size);  };
                    shellEditor->switchToHostApplication = [this]() { return owner.pluginHolder->switchToHostApplication(); };
                    shellEditor->getShouldOverrideSampleRateValue = [this]() { return &(owner.pluginHolder->getShouldOverrideSampleRateValue()); };
                    shellEditor->getAllowBluetoothInputValue = [this]() { return &(owner.pluginHolder->getAllowBluetoothInputValue()); };
                    shellEditor->getShouldCheckForNewVersionValue = [this]() { return &(owner.pluginHolder->getShouldCheckForNewVersionValue()); };
                    shellEditor->getRecentSetupFiles = [this]() { return &(owner.pluginHolder->getRecentSetupFiles()); };
                    shellEditor->getLastRecentsFolder = [this]() { return &owner.pluginHolder->getLastRecentsFolder(); };
                }
